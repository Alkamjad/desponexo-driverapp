...
// Safe, cross-platform haptic helper (no external imports)
const safeHapticImpact = () => {
  try {
    const C = typeof window !== 'undefined' ? window.Capacitor : undefined;
    const isNative = !!C?.isNativePlatform?.();
    if (isNative && C?.Plugins?.Haptics?.impact) {
      C.Plugins.Haptics.impact({ style: 'Medium' });
      return;
    }
